...
class IssueStatusChangeTimelineEvent extends TimelineEvent {
	...
	private function change_type() {
		...
		if( $this->old_status < $t_closed && $this->new_status >= $t_closed ) {
			...
			return IssueStatusChangeTimelineEvent::STATUS_CHANGE;
		}
	}
	...
	public function html() {
		switch( $this->type ) {
			...
			case IssueStatusChangeTimelineEvent::REOPENED:
				...
				break;
			case IssueStatusChangeTimelineEvent::STATUS_CHANGE:
				$t_html = $this->html_start('fa-edit');
				$t_string = sprintf(
				    lang_get( 'timeline_issue_status_changed' ),
                    prepare_user_name( $this->user_id ),
                    get_enum_element('status', $this->new_status),
                    string_get_bug_view_link( $this->issue_id )
                );
				break;
			default:
